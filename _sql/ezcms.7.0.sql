...
--
-- Generation Time: Aug 10, 2026
-- PHP Version: 8.0+
--
-- Fresh install schema for ezCMS 7. To create a new database:
--   DROP DATABASE IF EXISTS ezcms_db;
--   CREATE DATABASE ezcms_db;
--   USE ezcms_db;
-- then import this file. To upgrade an existing 6.x install run
-- `_sql/upgrade.6-to-7.sql` instead (it only adds/cleans indexes).
...
--   Revision history for file editors (layouts, includes, styles, scripts,
--   macros, controller). `createdby`/`createdon` are indexed so the per-user
--   activity log (users.php) and time-ordered lookups stay fast.
--
...
--   Redundant duplicate indexes from 6.x (`proprity`, `url_2`) removed.
--
...
--   `createdby` indexed so the per-user activity log (users.php) is covered.
--
