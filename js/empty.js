...
var ezCMSjs = {
	...
	processNext: function (macro, side1, side2) {
		...
		var tablepage = Math.ceil(idx/ezCMSjs.numbShown);
		if (!tablepage) tablepage = 1;
		if (parseInt($('#currpage').val())!=tablepage)
			ezCMSjs.drawPage(tablepage);
	...
	getPages: function() {
		...
		$.post( 'cdn.php?fetchall', $('#frmfind').serialize(), 
			function(data) {
				if (data.success) {
					var row, ico, badge;
	...
	drawPage: function (pageNumb) {
		...
		for(var i = numbStart; i < numbEnd; i++) {
			...
			else { ico = 'remove'; badge = 'important'; }
